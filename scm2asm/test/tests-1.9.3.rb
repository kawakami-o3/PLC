  ...
  '(let ([n 3])
    (let ([v0 (make-string n)])
     (let ([v1 (make-string (string-length v0))])
       (string-set! v0 (- (string-length v0) 3) #\a)
       (string-set! v0 (- (string-length v1) 2) #\b)
       (string-set! v0 (- (string-length v0) 1) #\c)
       (string-set! v1 (- (string-length v1) 3) #\Z)
       (string-set! v1 (- (string-length v0) 2) #\Y)
       (string-set! v1 (- (string-length v1) 1) #\X)
       (cons v0 v1))))' =>  "(\"abc\" . \"ZYX\")\n",



  '(let ([n 1])
     (string-set! (make-string n) (sub1 n) (integer->char 34))
     n)' => "1\n",
  '(let ([n 1])
     (let ([v (make-string 1)])
       (string-set! v (sub1 n) (integer->char n))
       (char->integer (string-ref v (sub1 n)))))' => "1\n",
  '(let ([v0 (make-string 1)])
     (string-set! v0 0 #\a)
     (let ([v1 (make-string 1)])
       (string-set! v1 0 #\A)
       (string-set! (if (string? v0) v0 v1)
            (sub1 (string-length (if (string? v0) v0 v1)))
            (integer->char
              (add1
                 (char->integer
                   (string-ref
